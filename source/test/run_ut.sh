...
if [ ! -f "${PWD}/RdkbGMock/docker_scripts/export_var.sh" ]; then
 ...
 log "INFO" "Running make for gw_prov_gtest.bin..."
 ...
 if [ -f "./source/test/gw_prov_gtest.bin" ]; then
     log "INFO" "Running gw_prov_gtest.bin"
     ./source/test/gw_prov_gtest.bin
     log "INFO" "Completed Test Execution"
 else
     log "ERROR" "gw_prov_gtest.bin does not exist, cannot run tests"
     exit 1
 fi
